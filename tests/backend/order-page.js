...
import {WebDriverHelper as helper, WebDriverManager} from 'wp-e2e-webdriver';
import {Helper, StoreOwnerFlow} from 'wc-e2e-page-objects';
...
const ITEM_SEARCH_SELECTOR = By.xpath('//div[@id="wc-backbone-modal-dialog"]//span[contains(@class, "select2")]');
const TAXCLOUD_STATUS_SELECTOR = By.xpath('//div[@id="sales_tax_meta"]//div[@class="inside"]/div');
...
test.describe('Order Page Tests', function () {
    const addProduct = (itemsBox, productTitle) => {
        ...
        return helper.mouseMoveTo(driver, ADD_ITEM_SELECTOR).then(() => {
            ...
            return Helper
                .select2OptionWithSearch(driver, ITEM_SEARCH_SELECTOR, productTitle, productTitle)
                .then(() => {
